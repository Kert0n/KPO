...
import { waitForDocumentAnimations, waitForStableRect } from './viewport'
...
async function waitForVueHydration(page: Page): Promise<void> {
  await page.waitForFunction(() => {
    const app = document.querySelector('#app') as (HTMLElement & { __vue_app__?: unknown }) | null
...
export async function waitForMermaid(page: Page, options: { requireDiagrams?: boolean } = {}): Promise<void> {
  await page.locator('.vp-doc').first().waitFor({ state: 'attached' })

  const renderingDisabled = await page.evaluate(
    () => document.documentElement.dataset.kpoTestMermaid === 'off'
  )
  ...
  await page.waitForFunction((requireDiagrams) => {
    ...
    return diagrams.every((diagram) => {
      if (diagram.getAttribute('data-kpo-render-state') === 'loading') return false
...
export async function expectNoPageOverflowFromVpDoc(page: Page): Promise<void> {
  const result = await page.evaluate(() => {
    ...
    const layout = [...document.querySelectorAll('body > *, .Layout, .VPNav, .VPSidebar, .VPContent')].map(
      (element) => {
        const node = element as HTMLElement
        const rect = node.getBoundingClientRect()
        return {
          selector: `${node.tagName.toLowerCase()}.${String(node.className)}`,
    ...
    // Chromium on macOS can reserve a dark-scrollbar gutter on the inline start
    // while still reporting it in scrollWidth. It shifts the whole app by exactly
    // the same amount and is not overflow produced by document content.
    const browserScrollbarGutter =
      appRoot &&
      appRootRect &&
      appRoot.clientWidth === document.documentElement.clientWidth &&
      Math.round(appRootRect.left) === overflow
        ? overflow
        : 0
    const documentState = {
    ...
    return {
      overflow,
      effectiveOverflow: overflow - browserScrollbarGutter,
      offenders,
      layout,
      documentState
    }
  ...
  expect(
    result.effectiveOverflow,
    JSON.stringify(
      { offenders: result.offenders, layout: result.layout, documentState: result.documentState },
      null,
      2
    )
  ).toBe(0)
...
async function hideSidebar(page: Page): Promise<void> {
  ...
  await waitForStableRect(page.locator('.VPDoc'))
}
...
export type ThemeSuiteCategory =
  | 'navigation'
  | 'responsive-layout'
  | 'code-switcher'
  | 'playground'
...
function suiteCategory(title: string): ThemeSuiteCategory {
  if (/ask ai|provider|clipboard|chatgpt|claude/i.test(title)) return 'ask-ai'
  if (/search/i.test(title)) return 'search'
  if (/playground/i.test(title)) return 'playground'
  if (/language|code switcher|author default/i.test(title)) return 'code-switcher'
  if (/mermaid/i.test(title)) return 'mermaid'
  if (/table/i.test(title)) return 'adaptive-tables'
  if (/navbar|navigation|sidebar toggle/i.test(title)) return 'navigation'
  return 'responsive-layout'
}

export type ThemeTestCase = {
  title: string
  body: (args: { page: Page }, testInfo: TestInfo) => unknown
}

export function themeSuiteCases(category: ThemeSuiteCategory): ThemeTestCase[] {
  const cases: ThemeTestCase[] = []
  const test = (title: string, body: (args: { page: Page }, testInfo: TestInfo) => unknown): void => {
    if (suiteCategory(title) === category) cases.push({ title, body })
  }
  ...
  test('desktop ask ai flyout has a vertical divider separating it from navigation', async ({ page }) => {
    await clearStorage(page)
    await page.setViewportSize({ width: 1600, height: 900 })
    await page.goto('')
  ...
  test('ask ai provider menu keeps selected and hover states clear', async ({ page }) => {
    ...
    const selectedBackground = await chatGpt.evaluate((node) => getComputedStyle(node).backgroundColor)
    ...
    const deepSeekBackground = await deepSeek.evaluate((node) => getComputedStyle(node).backgroundColor)
  ...
  test('mobile ask ai provider changes and persists in default vitepress screen', async ({ page }) => {
    await clearStorage(page)
    await page.setViewportSize({ width: 390, height: 844 })
    await page.goto('intro')
    await waitForVueHydration(page)
  ...
  test('ask ai waits for prompt preparation before first clipboard copy', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      ;(window as unknown as { __kpoCopyAttempts: Array<Record<string, unknown>> }).__kpoCopyAttempts = []
      Object.defineProperty(navigator, 'clipboard', {
        configurable: true,
        value: {
          writeText: async (text: string) => {
    ...
    const attempts = await page.evaluate(() => {
      return (window as unknown as { __kpoCopyAttempts: Array<Record<string, unknown>> }).__kpoCopyAttempts
    })
  ...
  test('ask ai starts clipboard copy before opening provider tab', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      Object.defineProperty(navigator, 'clipboard', {
        ...
        value: {
          writeText: async () => {
            ;(window as unknown as { __kpoSideEffectOrder: string[] }).__kpoSideEffectOrder.push('copy')
          }
        }
      })
  ...
  test('ask ai manual prompt appears only after clipboard methods fail', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      Object.defineProperty(navigator, 'clipboard', {
        ...
        value: {
          writeText: async () => {
            ;(window as unknown as { __kpoCopyAttempts: string[] }).__kpoCopyAttempts.push('clipboard-api')
            throw new DOMException('blocked', 'NotAllowedError')
          }
        }
      })
      document.execCommand = ((command: string) => {
        ;(window as unknown as { __kpoCopyAttempts: string[] }).__kpoCopyAttempts.push(`textarea-${command}`)
        return false
  ...
  test('playground=off blocks never initialize Playground', async ({ page }) => {
    await setStorage(page, {
      'kpo:code-language': 'kotlin',
      'kpo:playground-mode': '1'
    })
  ...
  test('language click preserves viewport position inside the interacted code block', async ({ page }) => {
    ...
    await waitForStableRect(switcher)
  ...
  test('playground toggle preserves viewport position inside the interacted code block', async ({ page }) => {
    ...
    await waitForStableRect(switcher, { frames: 3 })
    const after = await measureViewportRelativeTo(switcher)
  ...
  test('manual mermaid scroll is preserved across zoom and reset recenters', async ({ page }) => {
    ...
    await diagram.getByRole('button', { name: 'Увеличить диаграмму' }).click()
    await waitForStableRect(viewport)
    await expectNoPageOverflowFromVpDoc(page)
  ...
  test('fixture mermaid dark theme keeps label colors readable and token-based', async ({ page }) => {
    ...
    const revisions = await page
      .locator('.kpo-mermaid')
      .evaluateAll((nodes) => nodes.map((node) => Number(node.getAttribute('data-kpo-render-revision'))))
    await page.locator('.VPSwitchAppearance').first().click()
    await expect
      .poll(async () => {
        ...
        page
          ...
          .evaluateAll(
            (nodes, previous) =>
              nodes.every(
                (node, index) =>
                  Number(node.getAttribute('data-kpo-render-revision')) > (previous[index] ?? 0)
              ),
            revisions
          )
      )
  ...
  for (const selector of [
    '.kpo-wide-block--mermaid',
    '.kpo-wide-block--code',
    '.kpo-content-block--table',
    '.vp-doc p:has(> img:only-child)',
    '.kpo-playground'
  ]) {
    test(`wide element expands with hidden sidebar: ${selector}`, async ({ page }) => {
      await page.setViewportSize(LAYOUT_VIEWPORTS.desktop)
      await page.goto(UI_FIXTURE_ROUTE)
      await waitForMermaid(page, { requireDiagrams: true })
      await waitForAdaptiveTables(page)
      await expect.poll(async () => page.locator(selector).count()).toBeGreaterThan(0)
      const openWidth = await measureFirstWidth(page, selector)
      await hideSidebar(page)
      const hiddenWidth = await measureFirstWidth(page, selector)
      expect(hiddenWidth, selector).toBeGreaterThan(openWidth + 100)
      await expectNoPageOverflowFromVpDoc(page)
    })
  }
  ...
  for (const selector of [
    ...
    test(`mobile content stays viewport-contained: ${selector}`, async ({ page }) => {
      await page.setViewportSize(LAYOUT_VIEWPORTS.mobilePhone)
      await page.goto(UI_FIXTURE_ROUTE)
      await waitForMermaid(page, { requireDiagrams: true })
      await expect.poll(async () => page.locator(selector).count()).toBeGreaterThan(0)
