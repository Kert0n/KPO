...
Проект рассчитан на Node 24 (`.node-version`, `package.json#engines`). Скрипты `prebuild`, `pretest` и `pretest:ui` проверяют major-версию Node, чтобы локальные проверки не проходили случайно на Node 20/26.
...
- `{default=go}` — авторский язык по умолчанию для этого блока. Он сильнее сохранённого глобального языка, но читатель всё ещё может переключить конкретный блок вручную.
...
- **wide lane** — Mermaid, `multi-code`, одиночные fence-блоки, markdown-таблицы, playground и image-only paragraphs. Они центрируются в доступной области и владеют своим локальным scroll, если контент шире экрана.

Инвариант: страница не должна получать горизонтальный scroll. Широкий контент скроллится только внутри своего блока; глобальный `overflow-x: hidden` не используется.
