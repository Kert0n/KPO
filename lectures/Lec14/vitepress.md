...
Контейнеризация помогает упаковать приложение вместе с его runtime-зависимостями в переносимый image. Это не заменяет
архитектуру и не делает сервис автоматически масштабируемым, но решает важную DevOps-проблему: артефакт, который прошел
проверки в pipeline, можно почти тем же способом запустить в staging и production.
...
Границы ролей зависят от компании. В небольшой команде один человек может заниматься CI, облаком, мониторингом и
безопасностью. В крупной организации эти зоны часто разделены.
...
::: only kotlin
В Kotlin/JVM observability обычно приходит через JVM-экосистему: Micrometer, OpenTelemetry SDK/agent, логгеры вроде
Logback. Важно не только подключить библиотеку, но и передавать trace/context через корутины и внешние вызовы.
:::
...
::: only go
В Go observability обычно собирают явно: middleware измеряет duration/status, logger получает context, trace id
прокидывается через `context.Context`. Это требует дисциплины, зато хорошо показывает путь запроса в коде.
:::
...
sequenceDiagram
    ...
    Gateway->>Order: span: create order
    Order->>Payment: span: charge card
    Payment->>DB: span: write payment
    DB-->>Payment: ok
...
часть времени ждал Payment Service, а Payment Service ждал базу данных. Это меняет место расследования.
...
| Практика | Что проверяет | Где встраивается |
|---|---|---|
| SAST | уязвимые паттерны в исходном коде | merge request, CI |
| SCA / dependency scanning | уязвимости и лицензии зависимостей | CI, dependency update bot |
| Secret scanning | случайно закоммиченные ключи | pre-commit, repository scan, CI |
| Container image scanning | уязвимости в base image и пакетах | после сборки image |
| DAST | поведение работающего приложения снаружи | staging или test environment |
| IaC scanning | небезопасные настройки инфраструктуры | review Terraform/Kubernetes manifests |
| Compliance checks | соответствие политикам и стандартам | pipeline gates, audit reports |
