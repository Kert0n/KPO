...
NATS и ZeroMQ ближе к lightweight messaging и socket-style коммуникации, чем к тяжелому брокеру задач в классическом
понимании.

NATS хорош, когда нужна низкая задержка, простая модель публикации и подписки, сервисная коммуникация, а требования к
durability зависят от конкретной конфигурации и использования JetStream.

ZeroMQ - это скорее библиотека messaging-паттернов, чем отдельный managed broker. Она помогает строить push-pull,
pub-sub и request-reply поверх сетевых соединений, но многие эксплуатационные гарантии придется проектировать самому.

Выбирать такие решения стоит, когда их модель действительно совпадает с задачей: легкая коммуникация, низкая задержка,
контроль над сетью и понятные требования к потере сообщений.
...
Следующая лекция продолжит тему надежности: Saga, orchestration, choreography, Outbox, Inbox и почему двухфазный коммит
редко является хорошим универсальным ответом для микросервисов.
