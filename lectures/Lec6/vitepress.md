...
GoF делит паттерны на три группы.
...
flowchart TD
  ...
  Behavioral --> BGoal[Коммуникация и алгоритмы]
  Structural --> SGoal[Связи между объектами]
```

Структурные паттерны часто используют одну и ту же базовую технику - **композицию** или **агрегацию**. Объект хранит
...
```mermaid
flowchart LR
  Consumer[Consumer] --> Repo[Repository interface]
  Repo <|.. Real[RealRepository]
  Repo <|.. Proxy[CachingRepositoryProxy]
  Proxy --> Real
  Container[DI container] --> Proxy
  Container -. injects .-> Consumer
```
...
После самостоятельного изучения лекции вы должны уметь:

- объяснить, чем структурные паттерны отличаются от порождающих и поведенческих;
- отличить Adapter от Proxy и Decorator;
- объяснить, почему Facade может стать божественным объектом;
- показать, как Decorator заменяет комбинаторное наследование;
- объяснить внутреннее и внешнее состояние Flyweight;
- назвать ситуации, где Flyweight не стоит применять без профилирования;
- выбрать структурный паттерн по описанию проблемы;
- связать структурные паттерны с композициями, агрегациями и принципами SOLID.
