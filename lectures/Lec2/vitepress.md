...
Property injection похож на setter injection, но выглядит еще менее заметно: зависимость просто назначается через
свойство. В языках без свойств этот вариант отсутствует или выражается обычным setter-методом.

Главный риск тот же: если свойство обязательно, объект может быть создан и использован до настройки. Поэтому property
injection стоит применять только там, где это диктует фреймворк или зависимость действительно необязательная.
...
Composition Root отвечает за выбор конкретных реализаций. Например, в production он может собрать `OrderService` с
`PostgresOrderRepository`, а в тесте - с `InMemoryOrderRepository`.
...
передает их в конструкторы. В Android или серверных фреймворках эту роль может выполнять модуль DI, но правило остается
тем же: доменный объект не должен импортировать контейнер.
:::

::: only csharp
В ASP.NET Core composition root обычно находится в `Program.cs`: там регистрируют `AddScoped`, `AddSingleton`,
`AddTransient`, а контроллеры и сервисы получают зависимости через конструкторы. Важно не передавать `IServiceProvider`
в доменную модель как скрытый Service Locator.
:::
