...
Конструирование программного обеспечения - это работа не только с синтаксисом языка, но и с формой будущей системы: ее
объектами, зависимостями, границами ответственности и способностью меняться. Эта лекция вводит базовый словарь курса:
...
почему один и тот же работающий код может быть удобным для развития или, наоборот, быстро стать препятствием для
команды.
...
Дальше в курсе мы будем снова и снова возвращаться к одному и тому же движению. Сначала появляется маленькое приложение:
...
Эта первая лекция дает язык для такого разговора. Если ниже встречается термин, который подробно раскрывается позже,
здесь будет только рабочий минимум и ссылка на нужное место: внедрение зависимостей разбирается в
[Лекции 2](/lectures/02#dip-и-di), тестируемость - в [Лекции 3](/lectures/03#что-такое-unit-тест), доменная модель - в
[Лекции 7](/lectures/07#зачем-ddd), архитектурные границы - в [Лекции 8](/lectures/08#ports-and-adapters).

## Worked example: изменение ломает простой дизайн

### Ситуация

В учебном сервисе уведомлений сначала есть только email. Код работает, пользователь получает письмо, команда довольна.
Через неделю появляется SMS, потом push, потом требование логировать каждую отправку и отключать канал без релиза.

### Наивное решение

Добавить в один класс `NotificationService` поля `smtpClient`, `smsClient`, `pushClient`, несколько `if` по типу канала
и пару флагов конфигурации. Для первой версии это выглядит быстрее всего: один файл, один метод, все рядом.

### Что ломается

У класса появляются разные причины для изменения: текст письма, формат SMS, выбор канала, логирование, retry и
конфигурация. Тесты начинают требовать реальные клиенты или сложный setup. Новое требование уже не добавляется рядом с
предыдущим, а задевает весь метод.

### Улучшение

Разделить модель уведомления, выбор канала и конкретные отправители. Бизнес-код работает с контрактом отправки, а
конкретные реализации подключаются выше. Пока это только идея; в [Лекции 2](/lectures/02#composition-root) она станет
практикой DI и composition root.

### Почему это работает

Такой пример заранее показывает смысл всех базовых слов этой лекции: ответственность, связанность, интерфейс,
композиция, DIP и стоимость изменения. Мы не вводим абстракции ради красоты. Мы убираем место, где следующее изменение
заставило бы переписывать уже работающий код.
...
Работающий код - минимальное требование. Но в реальном проекте требования меняются: сегодня нужен вывод в консоль,
завтра PDF, послезавтра веб-страница; сегодня данные лежат в памяти, завтра в PostgreSQL; сегодня один канал
уведомлений, завтра несколько.
...
логикой и инфраструктурой. Но долгосрочно именно эти границы определяют, сможет ли команда быстро реагировать на новые
требования.

## Парадигмы

Парадигма - это не только набор приемов. Важнее, что она запрещает или ограничивает. Ограничение снижает хаос:
программист не может делать "как угодно", зато получает более предсказуемый код.
...
::: only go
В Go нет классов в привычном смысле, но есть структуры, методы и интерфейсы. Это делает язык хорошим примером того, что
объектный дизайн не обязан выглядеть одинаково во всех языках.
:::
...
В маленькой программе можно смотреть на классы отдельно. В приложении интереснее другое: как типы связаны между собой.
...
Эта схема не заменяет мышление, но помогает не начинать с наследования по инерции. Наследование - самый жесткий вариант
связи: потомок наследует не только полезное поведение, но и ожидания, ограничения, историю изменений родителя.
...
переопределяется потомками. Интерфейс удобен для способности: печатать, двигаться, сохранять, отправлять уведомление.

## LSP

Принцип подстановки Лисков говорит простыми словами: если код работает с базовым типом, он не должен ломаться при
...
Архитектурная работа часто проигрывает в споре за внимание, потому что ее результат труднее показать немедленно. Кнопку,
отчет или экран видно сразу. Границу между доменной логикой и инфраструктурой видно хуже, пока не приходит изменение
требований.
...
Архитектура чаще попадает в область "не срочно, но важно". Если ее постоянно откладывать, будущие срочные задачи станут
дороже. Но если заниматься только архитектурой и не поставлять работающие функции, проект тоже не выживет. Поэтому цель
курса - научиться выбирать минимальные архитектурные решения, которые реально снижают стоимость ближайших изменений.
...
Пример: нужно посчитать штрафные круги биатлониста. Если бизнес-правило звучит "один промах = один штрафной круг",
функция должна отражать именно это.
...
Не нужно добавлять погоду, кофе, гороскоп, стратегию трассы и фабрику штрафов, если правила задачи этого не требуют. Но
если правило расчета используется в десяти местах, DRY подсказывает вынести его в одну функцию, чтобы изменение правила
не размазывалось по проекту.
...
На этой странице мы говорили, что высокоуровневый код должен зависеть от абстракций. Но сама формулировка DIP еще не
объясняет, как конкретный объект попадет внутрь класса. Если `BookService` зависит от `Printer`, кто создаст
`ConsolePrinter`, `HtmlPrinter` или тестовый `BufferPrinter`? Где должен жить этот выбор, чтобы бизнес-правило не
знало о консоли, HTML и PDF?

Именно этот вопрос открывает [Лекцию 2 про DI](/lectures/02#проблема): зависимость недостаточно объявить интерфейсом, ее
нужно передать в правильном месте.
