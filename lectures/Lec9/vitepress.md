...
|---|---|---|---|
| Key-value | `sessionId -> session data` | Сессии, токены, счетчики, feature flags | Сложно делать сложные запросы по содержимому |
| Document | Заказ со строками, адресом и историей статусов одним документом | Агрегаты с гибкой структурой, JSON-подобные данные | Дублирование и сложные обновления связанных данных |
| Column-family | События, метрики, временные ряды | Очень большие объемы записей с предсказуемыми запросами | Модель проектируется под запросы заранее |
| Graph | Пользователи, связи, рекомендации, маршруты | Запросы по отношениям между объектами | Не лучший выбор для обычного CRUD |
| Search / vector | Документы, эмбеддинги, поиск похожих объектов | Поиск, ранжирование, RAG, рекомендации | Обычно требует синхронизации с основной БД |
...
Распределенные SQL-системы, их также часто называют NewSQL, пытаются совместить знакомую SQL-модель с горизонтальным
...
Что такие системы обещают:

- сохранить SQL и привычную модель запросов;
- распределить данные по нескольким узлам;
- автоматизировать репликацию и маршрутизацию;
- дать горизонтальное масштабирование без ручного шардирования в прикладном коде.

Но распределенность не бывает бесплатной. Консенсус, распределенные транзакции, сетевые задержки, rebalancing и
наблюдаемость имеют цену. Для маленького проекта distributed SQL может быть медленнее, дороже и сложнее, чем обычный
PostgreSQL с хорошими индексами, read replica, бэкапами и мониторингом.

::: tip Практическое правило
Начинайте с простой надежной модели. Распределенную SQL-БД стоит рассматривать, когда у вас есть реальные симптомы:
географическое распределение, требования к автоматическому failover, объемы или нагрузка, которые плохо укладываются в
один узел или обычную репликацию.
:::
