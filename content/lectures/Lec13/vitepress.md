...
Сервис запущен в 5 репликах. Каждая реплика кэширует карточку товара локально. Пользователь обновил цену через
реплику 1. Реплики 2-5 отдают старую цену, пока TTL не истечёт. Это — главная проблема in-process cache в
распределённом окружении.
...
::: multi-code "Cache-aside: чтение товара"
...
::: multi-code "TTL cache: запись с временем жизни"
...
::: multi-code "Negative caching: NotFound на короткий TTL"
...
::: only go
Паттерн singleflight появился в Go (`golang.org/x/sync/singleflight`) и стал first-class citizen экосистемы.
Другие языки портировали идею, но в Go она решается одной строкой:
...
func GetProduct(id string) (Product, error) {
    val, err, _ := group.Do("product:"+id, func() (any, error) {
        return repo.Load(id)
    })
...
`group.Do` гарантирует: пока первый вызов для ключа `"product:42"` не вернулся, все остальные горутины ждут тот
же результат. Не нужен внешний mutex, не нужен ручной `inFlight` map.
:::

::: only csharp
В C# библиотеки **FusionCache** и **LazyCache** реализуют cache-aside + stampede protection + fallback из коробки:

```csharp
// FusionCache: stampede protection встроена
var product = await cache.GetOrSetAsync(
    $"product:{id}",
    async _ => await repository.FindById(id),
    new FusionCacheEntryOptions(TimeSpan.FromMinutes(5))
        .SetFailSafe(true, TimeSpan.FromHours(1)) // stale fallback
);
```

FusionCache автоматически делает single flight для одинаковых ключей и поддерживает fail-safe fallback на stale
данные, если основной источник недоступен.
:::
...
несколько структур данных и удобен как общий кэш для нескольких экземпляров приложения. Паттерны важнее продукта,
но Redis удобно связывает их воедино:

- **Redis pub/sub** → event-driven invalidation из предыдущего раздела: один сервис публикует «цена изменилась»,
  все подписчики сбрасывают ключ;
- **Redis Lua** → atomic read-modify-write для single flight: проверка и запись в одном round-trip;
- **Redis TTL** → cache-aside с автоматическим expiration, без ручного cleanup.
...
## От кэша к CQRS {#от-кэша-к-cqrs}

Кэш ускоряет повторное чтение. Но что если проблема не в скорости, а в ФОРМЕ данных? Клиенту нужна карточка с
агрегированными отзывами, персональной ценой и промо-статусом — join из 6 таблиц, 3 внешних вызовов и 2 вычислений.
Кэшировать этот ответ можно, но каждое изменение любого из источников инвалидирует весь кэш. Это другая задача.
...
Прежде чем погружаться в варианты реализации, стоит отсечь случаи, когда CQRS — лишняя сложность.
...
Если ваш случай не подпадает под эти исключения — ниже три варианта реализации, от простого к сложному.
