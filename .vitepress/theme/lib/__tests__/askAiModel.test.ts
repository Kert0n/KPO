...
describe('askAiModel', () => {
  ...
  it('collects before bridges and substantive blocks in natural order', () => {
    ...
    expect(collectBeforeContextBlocks(blocks, 2, 1800).map((item) => item.id)).toEqual(['useful', 'bridge'])
  })
...
const grpcContext: AskAiPageContext = {
  ...
  blocks: [
    block(
      'grpc-before',
      'paragraph',
      'gRPC - популярная RPC-реализация. В ней контракт обычно описывают в `.proto`-файле, затем генерируют серверные и клиентские stubs для нужных языков. Сообщения сериализуются через Protocol Buffers, а транспорт обычно работает поверх HTTP/2.'
    ),
    block(
      'grpc-proto',
      'code',
