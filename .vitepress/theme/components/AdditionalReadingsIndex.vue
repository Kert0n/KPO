...
<template>
  <div class="kpo-additional-readings">
    <section
      ...
      :key="lecture.lecture"
      class="kpo-additional-readings__lecture"
    >
      <header class="kpo-additional-readings__header">
        <h2 :id="`lecture-${lecture.lecture}`">
          Лекция {{ lecture.lecture }}. {{ lecture.title }}
        </h2>
        <a class="kpo-additional-readings__lecture-link" :href="withBase(lecture.url)">
          Открыть лекцию
        </a>
      </header>

      <div
        v-for="group in lecture.groups"
        :key="`${lecture.lecture}-${group.title}`"
